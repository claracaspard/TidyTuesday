...
word_counts <- romeo_juliet %>%
  ...
  filter(!str_detect(word, "^[0-9]+$")) %>%  # Remove numbers, if you detect them in the column word
...
romeo|>
  ggplot(aes(label = word, size = n, color = n)) + 
  ggwordcloud::geom_text_wordcloud_area(mask=readPNG("D:/shakespeare.png"), rm_outside = TRUE)
```
